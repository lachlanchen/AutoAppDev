...
- task_seq: 44
- task_slug: aaps_numbered_prefix_parser
- area: backend
- title: Parser: accept numbered prefixes
- acceptance: `backend/pipeline_parser.py` accepts optional numeric prefixes before KEYWORD (e.g. `1.2 STEP {...}`) while remaining deterministic; add an example under `examples/` and verify parse via a small python snippet
