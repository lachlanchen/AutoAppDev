...
- task_seq: 49
- task_slug: runner_outbox_progress_helper
- area: scripts
- title: Runner: outbox progress helper
- acceptance: Runner template adds a helper to write progress updates to `runtime/outbox/` (no HTTP required) and meta-round loop uses it so the PWA can observe progress
